...
{
    BOOL isSuccess = [self writeToFile:[NSString stringWithFormat:@"/Users/oyo/Desktop/%@.plist", plistName] atomically:YES]; // 调试用
    
    if (isSuccess) {
        NSLog(@"写入到桌面Plist文件成功");
    }else{
        NSLog(@"写入到桌面Plist文件失败，请检查路径是否正确");
    }
}
